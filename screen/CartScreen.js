    ...
    return (
        <View className="bg-white flex-1">
            ...
            <View style={{ backgroundColor: themeColors.bgColor(0.2) }}
                ...
                <Image source={require('../assets/images/Deli.png')} className="w-20 h-20 rounded-full" />
                <Text className="text-l pl-4">
            ...
            <ScrollView
                ...
                className="bg-white pt-5">
                {
                   Object.entries(groupedItems).map(([key, items]) => {
                    let dish = items[0];
                        return (
                            <View key={key} className="flex-row items-center space-x-3 py-3 px-4 bg-white rounded-full">
                                <Text className="font-bold" style={{ color: themeColors.text }}>
                                    {items.length} x
                                </Text>
                                <Image source={dish.image} className="w-14 h-14 rounded-full" />
                                <Text className="flex-1 font-bold text-gray-700 ">{dish.name}</Text>
                                <Text className="font-bold" style={{ color: themeColors.text }}>${dish.price}</Text>
                                <TouchableOpacity
                                onPress={()=>dispatch(removeFormCart({id: dish.id}))}className="p-1 rounded-full" style={{ backgroundColor: themeColors.bgColor(1) }}>
